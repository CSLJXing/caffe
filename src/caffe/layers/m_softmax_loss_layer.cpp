...
void MSoftmaxWithLossLayer<Dtype>::LayerSetUp(
  ...
  thresh_ = this->layer_param_.loss_param().thresh();
...
template <typename Dtype>
void MSoftmaxWithLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void MSoftmaxWithLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      Dtype prob_value = prob_data[i * dim + label_value * inner_num_ + j];
      if (prob_value > thresh_)
      {
        continue;
      }
      ...
      loss -= weight_value * log(std::max(prob_value, Dtype(FLT_MIN)));
...
template <typename Dtype>
void MSoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        } else if (bottom_diff[i * dim + label_value * inner_num_ + j] > thresh_) {
          // prob_value > thresh_
          for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
            bottom_diff[i * dim + c * inner_num_ + j] = 0;
          }
...
#ifdef CPU_ONLY
STUB_GPU(MSoftmaxWithLossLayer);
#endif

INSTANTIATE_CLASS(MSoftmaxWithLossLayer);
REGISTER_LAYER_CLASS(MSoftmaxWithLoss);
