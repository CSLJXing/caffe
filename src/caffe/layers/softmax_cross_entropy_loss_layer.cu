...
__global__ void SoftmaxCrossEntropyLossForwardGPU(const int nthreads,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
      loss[index] = target_prob_data[index]
          * log(
              max(target_prob_data[index], Dtype(FLT_MIN)) /
              max(predict_prob_data[index], Dtype(FLT_MIN))
            );
  }
}
...
void SoftmaxWithCrossEntropyLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    // const int dim = predict_prob_.count() / outer_num_;
    // const int nthreads = outer_num_ * inner_num_;
    // // NOLINT_NEXT_LINE(whitespace/operators)
    // SoftmaxCrossEntropyLossBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
    //     CAFFE_CUDA_NUM_THREADS>>>(nthreads, predict_prob_data, target_prob_data,
    //     bottom_diff, dim, inner_num_);
    const Dtype loss_weight = top[0]->cpu_diff()[0];
    if (normalize_) {
      caffe_gpu_scal(predict_prob_.count(), loss_weight / (outer_num_ * inner_num_), bottom_diff);
    } else {
      caffe_gpu_scal(predict_prob_.count(), loss_weight / outer_num_, bottom_diff);
    }
  }
}
