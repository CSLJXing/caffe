...
{
  ROIPoolingParameter roi_align_param = this->layer_param_.roi_pooling_param();
                                   ...
                                   const vector<Blob<Dtype>*>& top) 
{
  ...
  vector<int> shape;
  shape.push_back(bottom[1]->num());
  shape.push_back(channels_);
  shape.push_back(pooled_height_);
  shape.push_back(pooled_width_);
  shape.push_back(4);
